...
class mathematica:
	...
	def multivariate(meanVec, covMatrix, Xin):
		total_features = len(meanVec.tolist())
		return ((1/m.sqrt((2*m.pi)**total_features)*abs(det(covMatrix)))*m.exp(-0.5*np.matmul(np.matmul((Xin-meanVec).T, inv(covMatrix)),(Xin-meanVec))))
	...
	def cov(perClassData, perClassMean):
		...
		for i in range(0, len(perClassData[0])):
			...
			for j in range(0,len(perClassData[0])):
				sum=0;
				for k in range(0, len(perClassData)):
					sum=sum+(perClassData[k][i]*perClassData[k][j])
				sum=sum/len(perClassData)
				sum=sum-perClassMean[i]*perClassMean[j]
				l1.append(sum)
			l2.append(l1)
